...
public class ChatTest extends VlcjTest {
	...
	public ChatTest() {
		...
		sourceControls
				.setLayout(new BoxLayout(sourceControls, BoxLayout.X_AXIS));
		...
		localStreamControls.setLayout(new BoxLayout(localStreamControls,
				BoxLayout.X_AXIS));
		...
		remoteStreamControls.setLayout(new BoxLayout(remoteStreamControls,
				BoxLayout.X_AXIS));
	...
	private void start() {
		mediaTextField.setText(!RuntimeUtil.isWindows() ? "v4l2:///dev/video0"
				: "dshow://");
	...
	private void send() {
		...
		if (mrl.length() > 0) {
			...
			if (parts.length == 2) {
				...
				String[] localOptions = { formatRtpStream(host, port),
				...
				JOptionPane.showMessageDialog(frame,
						"You must specify host:port to stream to.", "Error",
			...
			JOptionPane
					.showMessageDialog(
							frame,
							"You must specify source media, e.g. v4l2:///dev/video0 on Linux or dshow:// on Windows.",
